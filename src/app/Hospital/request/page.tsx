...
import { RequestResponse, displayRequest } from "@/hospital.types";
...
export default function Hospitalreq() {
  ...
  const [requestData, setRequestData] = useState<RequestResponse[]>([]);
  const itemsPerPage = 6; // Set items per page to 6
  console.log("Hello"); // delete later

  useEffect(() => {
    const fetchData = async () => {
      var storedData = await getHospitalRequests();

      if (storedData) {
        console.log(storedData);
        setRequestData(storedData.data as RequestResponse[]);
      } else {
        console.log("empty");
      }
    };
  ...
  const displayData = requestData.slice(
